...
def wiki_search(search_query):
    response = requests.get(
        "https://en.wikipedia.org/w/api.php?action=query&list=search&prop=info&inprop=url&utf8=&format=json&origin=*&srlimit=20&srsearch={}".format(search_query))
...
def dbpedia_search(search_query):
    ...
    response = requests.get(
        "http://lookup.dbpedia.org/api/search/PrefixSearch?QueryString={}".format(search_query), headers=headers)
...
def lei_search(search_query):
    response = requests.get(
        "https://api.leilex.com/API/LEI/AutoComplete/?query={}&filterType=Name".format(search_query))
